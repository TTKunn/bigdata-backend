...
- **API前缀**：`/api/dev` (开发环境专用)
- **Swagger文档**：http://localhost:8080/swagger-ui.html (仅开发环境)

### 注意事项

1. 所有接口仅在开发环境(`dev`)中启用，生产环境不会暴露这些接口
2. 请求和响应数据格式均为JSON
...
- 请求名称：创建商品(Form Data)
- 请求方法：POST
- 请求URL：`http://localhost:8080/api/dev/product`
...
#### 请求信息
- **接口路径**：`PUT /api/dev/product/{productId}/stock`
- **请求方法**：PUT
...
#### 请求信息
- **接口路径**：`GET /api/dev/product/health`
- **请求方法**：GET
...
1. **开发环境限制**：所有接口仅在开发环境(`dev` profile)中可用
