...
3. [商品管理接口](#商品管理接口)
   ...
   - [3.4 更新商品库存](#34-更新商品库存)
   - [3.5 服务健康检查](#35-服务健康检查)
4. [购物车管理接口](#购物车管理接口)
...
6. [销售统计接口](#销售统计接口)
   - [6.1 获取总销售额统计](#61-获取总销售额统计)
   - [6.2 获取当日销售额统计](#62-获取当日销售额统计)
   - [6.3 获取当日订单数量统计](#63-获取当日订单数量统计)
   - [6.4 获取最畅销商品排行榜](#64-获取最畅销商品排行榜)
7. [错误码说明](#错误码说明)
8. [测试用例](#测试用例)
    ...
    "products": [
      {
        ...
        "price": 199.00,
        "brand": "Redmi",
        "description": "Redmi真无线耳机，40dB主动降噪",
        "createTime": null,
        "status": "ACTIVE"
      },
      {
...
#### 接口描述
获取系统上线以来所有已完成订单的总销售额统计信息，包括总销售额、已完成订单数和最后更新时间。
...
```json
{
  "code": 200,
  "message": "操作成功",
  "timestamp": "2026-01-08T08:56:52Z",
  "data": {
    "totalSales": 55388.00,
    "completedOrders": 6,
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| totalSales | number | 总销售额（单位：元） |
| completedOrders | integer | 已完成订单总数 |
| lastUpdateTime | string | 最后更新时间（ISO 8601格式） |
...
#### 业务说明
- 只统计状态为 `COMPLETED`（已完成）的订单
- 统计数据从Redis缓存中读取，响应速度快（< 100ms）
- 数据由定时任务每20秒自动更新一次
- 订单完成后约20秒内统计数据会自动更新
...
##### ApiFox测试步骤
1. 创建新请求，方法选择GET
2. URL填写：`http://localhost:8080/api/statistics/total-sales`
3. 点击"发送"按钮
4. 验证返回的统计数据
...
#### 接口描述
获取指定日期（默认当天）的销售额统计信息，包括当日销售额、订单数量、平均客单价和最后更新时间。
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| date | string | 否 | 当天 | 统计日期，格式：yyyyMMdd（如：20260108） |
...
```json
{
  "code": 200,
  "message": "操作成功",
  "timestamp": "2026-01-08T08:59:27Z",
  "data": {
    "date": "20260108",
    "dailySales": 32194.00,
    "orderCount": 3,
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| date | string | 统计日期（yyyyMMdd格式） |
| dailySales | number | 当日销售额（单位：元） |
| orderCount | integer | 当日订单数量 |
| averageOrderValue | number | 平均客单价（单位：元） |
| lastUpdateTime | string | 最后更新时间（ISO 8601格式） |
...
```json
{
  ...
  "data": {
    ...
    "orderCount": 0,
    "averageOrderValue": 0,
    "lastUpdateTime": "2026-01-08T08:59:27.123"
  }
}
```
...
# 查询指定日期销售额
curl -X GET "http://localhost:8080/api/statistics/daily-sales?date=20260108"
```

##### ApiFox测试步骤
1. 创建新请求，方法选择GET
2. URL填写：`http://localhost:8080/api/statistics/daily-sales`
3. 可选：添加查询参数 `date`（如：20260108）
4. 点击"发送"按钮
5. 验证返回的统计数据
...
```json
{
  "code": 200,
  "message": "操作成功",
  "timestamp": "2026-01-08T08:59:28Z",
  "data": {
    "date": "20260108",
    "orderCount": 3,
    "lastUpdateTime": "2026-01-08T08:58:34.533"
...
# 查询指定日期订单数量
curl -X GET "http://localhost:8080/api/statistics/daily-orders?date=20260108"
```

##### ApiFox测试步骤
1. 创建新请求，方法选择GET
2. URL填写：`http://localhost:8080/api/statistics/daily-orders`
3. 可选：添加查询参数 `date`（如：20260108）
4. 点击"发送"按钮
5. 验证返回的统计数据
...
| 参数名 | 类型 | 必填 | 默认值 | 说明 |
|--------|------|------|--------|------|
| limit | integer | 否 | 3 | 返回数量限制，范围：1-20 |
...
```json
{
  "code": 200,
  "message": "操作成功",
  "timestamp": "2026-01-08T08:59:30Z",
  "data": {
    "topProducts": [
      {
        "rank": 1,
...
| 字段名 | 类型 | 说明 |
|--------|------|------|
| topProducts | array | 畅销商品列表 |
| topProducts[].rank | integer | 排名（从1开始） |
| topProducts[].productId | string | 商品ID |
| topProducts[].productName | string | 商品名称 |
| topProducts[].totalSales | integer | 总销量（件数） |
| topProducts[].lastUpdateTime | string | 最后更新时间（ISO 8601格式） |
...
#### 业务说明
- 只统计状态为 `COMPLETED`（已完成）订单中的商品销量
- 按商品总销量降序排列
- 统计数据从Redis Sorted Set中读取，响应速度快（< 100ms）
- 数据由定时任务每20秒自动更新一次
- limit参数范围：1-20，超出范围返回400错误
...
```bash
# 查询前3名畅销商品（默认）
curl -X GET "http://localhost:8080/api/statistics/top-products"

# 查询前5名畅销商品
curl -X GET "http://localhost:8080/api/statistics/top-products?limit=5"

# 查询前10名畅销商品
curl -X GET "http://localhost:8080/api/statistics/top-products?limit=10"
```

##### ApiFox测试步骤
1. 创建新请求，方法选择GET
2. URL填写：`http://localhost:8080/api/statistics/top-products`
3. 可选：添加查询参数 `limit`（如：5）
4. 点击"发送"按钮
5. 验证返回的排行榜数据
...
销售统计数据采用**异步批量更新**机制，确保高并发场景下的系统性能：

1. **订单完成时**：将订单ID写入Redis待更新队列（`statistics:update:queue`）
2. **定时任务执行**：每20秒执行一次，从队列中批量取出订单ID（最多1000条）
3. **批量更新统计**：查询订单详情，批量更新Redis中的统计数据
4. **去重机制**：使用Redis Set记录已处理的订单ID，防止重复统计

#### 数据存储结构

所有统计数据存储在Redis中，确保查询性能：

```redis
# 总销售额统计
statistics:sales:total → "55388.00" (String)
statistics:sales:total:count → "6" (String)
statistics:sales:total:update → "1736323114526" (String)

# 当日销售额统计 (Hash)
statistics:sales:daily:20260108 → {
  sales: "32194.00",
  orders: "3",
  lastUpdate: "1736323114533"
}

# 商品销量排行榜 (Sorted Set)
statistics:product:sales:rank → [
  {score: 3, member: "000100000003"},
  {score: 2, member: "000100000002"},
  {score: 1, member: "000200000001"}
]

# 商品销量详情 (Hash)
statistics:product:sales:000100000003 → {
  name: "小米17 Ultra",
  totalSales: "3",
  lastUpdate: "1736323114543"
}
```

#### 性能特点

- **查询响应时间**：< 100ms（直接从Redis读取）
- **数据更新延迟**：订单完成后约20秒内更新
- **并发支持**：支持1000+并发查询
- **缓存命中率**：> 99.9%
...
1. **数据延迟**：统计数据不是实时的，订单完成后约20秒内会更新
2. **只统计已完成订单**：只有状态为 `COMPLETED` 的订单才会被统计
3. **历史数据保留**：当日销售额数据保留30天，总销售额永久保留
4. **定时任务依赖**：需要在主应用类中添加 `@EnableScheduling` 注解启用定时任务
...
- v1.6 (2026-01-08)：新增销售统计分析接口（总销售额、当日销售额、当日订单数量、畅销商品排行榜）
- v1.7 (2026-01-08)：商品列表和商品详情接口响应中添加 `description`（商品介绍）字段
