...
#### 接口描述
从用户购物车中的选中商品创建订单。系统会自动验证库存、扣减库存、创建订单记录，并清空购物车中已下单的商品。
...
1. **获取购物车选中商品**：从Redis获取用户购物车中selected=true的商品
2. **预检查库存**：验证所有商品的库存是否充足
3. **计算订单金额**：计算商品总金额和实付金额
4. **生成订单号**：使用Redis原子递增生成唯一订单号
5. **原子扣减库存**：使用Redis Lua脚本原子扣减库存
6. **创建订单记录**：保存订单到HBase order_history表
7. **清空购物车**：删除购物车中已下单的商品
8. **异常回滚**：如果任何步骤失败，自动回滚已扣减的库存
